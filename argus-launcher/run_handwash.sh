...
PROJECT_FILE="$PROJECT_DIR/automatic_rubbing.py"
IMAGE="dustynv/nanoowl:r36.3.0"
LOCK_FILE="/tmp/argus-handwash.lock"

exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "Argus Hand Wash is already running."
...
if [ "$STATUS" -ne 0 ]; then
    ...
    echo "Argus Hand Wash has stopped."
fi
